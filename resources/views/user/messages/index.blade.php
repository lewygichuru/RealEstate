                                    ...
                                    <tr class="{{ $message->status == 0 ? 'font-semibold' : '' }}">
                                        ...
                                        <td>
                                            <div class="flex gap-1">
                                                @if($message->status == 0)
                                                    <a href="{{ route('user.message.read', $message->id) }}" class="btn btn-warning btn-xs" title="Unread"><span class="material-icons text-sm">local_library</span></a>
                                                @else
                                                    <a href="{{ route('user.message.read', $message->id) }}" class="btn btn-success btn-xs" title="Read"><span class="material-icons text-sm">done</span></a>
                                                @endif
                                                <a href="{{ route('user.message.replay', $message->id) }}" class="btn btn-primary btn-xs" title="Reply"><span class="material-icons text-sm">reply</span></a>
                                                ...
                                                <form action="{{ route('user.messages.destroy', $message->id) }}" method="POST" id="del-message-{{ $message->id }}" class="hidden">
                                                    @csrf @method('DELETE')
                                                </form>
                                            </div>
                                        </td>
